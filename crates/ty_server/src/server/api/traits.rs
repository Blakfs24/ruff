...
pub trait SyncRequestHandler: RequestHandler {
    fn run(
        session: &mut Session,
        client: &Client,
...
pub trait RetriableRequestHandler: RequestHandler {
...
pub trait BackgroundDocumentRequestHandler: RetriableRequestHandler {
...
/// diagnostics.
pub trait BackgroundRequestHandler: RetriableRequestHandler {
    /// Processes the request parameters and returns the LSP request result.
    ///
    /// This is the main method that handlers implement. It takes the request parameters
    /// from the client and computes the appropriate response data for the LSP request.
...
/// notifications will be handled while `run` is executing. Try to avoid doing any I/O or
/// long-running computations.
pub trait SyncNotificationHandler: NotificationHandler {
    fn run(
        session: &mut Session,
        client: &Client,
...
pub trait BackgroundDocumentNotificationHandler: NotificationHandler {
